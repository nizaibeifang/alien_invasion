...
def update_scrren(ai_settings,screen,ship,aliens,bullets):
    '''更新屏幕上的图像，并切换到新屏幕上'''
    #每次循环时都重绘屏幕
...
def check_bullet_alien_collisions(ai_settings,screen,ship,aliens,bullets):
    ...
    '''遍历编组bullets中的每颗子弹，再遍历编组aliens中的每个外星人。每当有子弹和外星人的rect重叠时，
    groupcollide()就在它返回的字典中添加一个键 - 值对。两个实参True告诉Pygame删除发生碰撞的子弹和外星人'''
    if len(aliens) == 0:
        #删除现有的子弹并新建一群外星人
        bullets.empty()
...
def update_aliens(ai_settings,status,screen,ship,aliens,bullets):
    ...
    if pygame.sprite.spritecollideany(ship,aliens):
        #spritecollide 检查ship和aliens编组是否碰撞，并返回第一个与飞船发生碰撞的外星人
...
def check_aliens_bottom(ai_settings,status,screen,ship,aliens,bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            #像飞船撞到一样进行处理
            ship_hit(ai_settings,status,screen,ship,alien,bullets)
            break
